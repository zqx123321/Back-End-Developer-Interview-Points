...
<!-- TOC -->autoauto- [分布式](#分布式)auto    - [一、大型网站系统的特点](#一大型网站系统的特点)auto        - [高并发，大流量](#高并发大流量)auto        - [高可用](#高可用)auto        - [海量数据](#海量数据)auto        - [用户分布广泛，网络情况复杂](#用户分布广泛网络情况复杂)auto        - [安全环境恶劣](#安全环境恶劣)auto        - [需求快速变更，发布频繁](#需求快速变更发布频繁)auto        - [渐进式发展](#渐进式发展)auto    - [二、大型网站架构演化发展历程](#二大型网站架构演化发展历程)auto        - [初始阶段的网站架构](#初始阶段的网站架构)auto        - [应用服务和数据服务分离](#应用服务和数据服务分离)auto        - [使用缓存改善网站性能](#使用缓存改善网站性能)auto        - [使用应用服务器集群改善网站的并发处理能力](#使用应用服务器集群改善网站的并发处理能力)auto        - [数据库读写分离](#数据库读写分离)auto        - [使用反向代理和 CDN 加速网站响应](#使用反向代理和-cdn-加速网站响应)auto        - [使用分布式文件系统和分布式数据库系统](#使用分布式文件系统和分布式数据库系统)auto        - [使用 NoSQL 和搜索引擎](#使用-nosql-和搜索引擎)auto        - [业务拆分](#业务拆分)auto        - [分布式微服务](#分布式微服务)auto    - [三、拆分 VS 集群](#三拆分-vs-集群)auto    - [四、微服务 VS SOA](#四微服务-vs-soa)auto    - [五、前后端完全分离与Rest规范](#五前后端完全分离与rest规范)auto    - [六、CAP三进二和Base定理](#六cap三进二和base定理)auto        - [关系型数据库遵循ACID规则](#关系型数据库遵循acid规则)auto        - [CAP三进二](#cap三进二)auto        - [BASE定理](#base定理)auto        - [分布式一致性理论paxos、raft、zab算法](#分布式一致性理论paxosraftzab算法)auto- [中间件](#中间件)auto    - [一、缓存](#一缓存)auto        - [为什么要使用缓存](#为什么要使用缓存)auto        - [优秀的缓存系统Redis](#优秀的缓存系统redis)auto        - [redis为什么这么快](#redis为什么这么快)auto        - [redis的数据类型，以及每种数据类型的使用场景](#redis的数据类型以及每种数据类型的使用场景)auto        - [redis的过期策略以及内存淘汰机制](#redis的过期策略以及内存淘汰机制)auto        - [渐进式ReHash](#渐进式rehash)auto        - [渐进式rehash的原因](#渐进式rehash的原因)auto            - [渐进式rehash的步骤](#渐进式rehash的步骤)auto        - [缓存穿透](#缓存穿透)auto        - [缓存雪崩](#缓存雪崩)auto    - [二、消息队列](#二消息队列)auto        - [消息队列应用场景](#消息队列应用场景)auto            - [异步处理](#异步处理)auto            - [应用解耦](#应用解耦)auto            - [流量削锋](#流量削锋)auto            - [日志处理](#日志处理)auto            - [消息通讯](#消息通讯)auto        - [消息中间件示例](#消息中间件示例)auto            - [电商系统](#电商系统)auto            - [日志收集系统](#日志收集系统)auto        - [JMS消息服务](#jms消息服务)auto            - [消息模型](#消息模型)auto            - [消息消费](#消息消费)auto        - [防止消息丢失](#防止消息丢失)auto            - [同步的事务——停止等待](#同步的事务停止等待)auto            - [同步的事务——连续ARQ](#同步的事务连续arq)auto            - [异步的事务——回调机制](#异步的事务回调机制)auto        - [消息的幂等处理](#消息的幂等处理)auto        - [消息的按序处理](#消息的按序处理)auto    - [三、搜索引擎](#三搜索引擎)auto        - [概述](#概述)auto        - [特点（优势）：](#特点优势)auto        - [使用场景：](#使用场景)auto        - [倒排索引](#倒排索引)auto        - [创建索引](#创建索引)auto            - [一些要索引的原文档(Document)](#一些要索引的原文档document)auto            - [将原文档传给分次组件(Tokenizer)](#将原文档传给分次组件tokenizer)auto            - [将得到的词元(Token)传给语言处理组件(Linguistic Processor)](#将得到的词元token传给语言处理组件linguistic-processor)auto            - [将得到的词(Term)传给索引组件(Indexer)](#将得到的词term传给索引组件indexer)auto        - [搜索索引](#搜索索引)auto            - [用户输入查询语句](#用户输入查询语句)auto            - [对查询语句进行词法分析，语法分析，及语言处理](#对查询语句进行词法分析语法分析及语言处理)auto            - [搜索索引，得到符合语法树的文档](#搜索索引得到符合语法树的文档)auto            - [根据得到的文档和查询语句的相关性，对结果进行排序](#根据得到的文档和查询语句的相关性对结果进行排序)auto        - [Lucene和ElasticSearch](#lucene和elasticsearch)auto        - [分词器](#分词器)auto- [大数据与高并发](#大数据与高并发)auto    - [一、秒杀架构设计](#一秒杀架构设计)auto        - [业务介绍](#业务介绍)auto        - [业务特点](#业务特点)auto            - [瞬时并发量大](#瞬时并发量大)auto        - [](#)auto            - [库存量少](#库存量少)auto            - [业务简单](#业务简单)auto        - [技术难点](#技术难点)auto            - [现有业务的冲击](#现有业务的冲击)auto            - [直接下订单](#直接下订单)auto            - [页面流量突增](#页面流量突增)auto        - [架构设计思想](#架构设计思想)auto            - [限流](#限流)auto            - [削峰](#削峰)auto            - [异步](#异步)auto            - [缓存](#缓存)auto        - [整体架构](#整体架构)auto        - [客户端优化](#客户端优化)auto            - [秒杀页面](#秒杀页面)auto            - [防止提前下单](#防止提前下单)auto        - [API 接入层优化](#api-接入层优化)auto            - [限制用户维度访问频率](#限制用户维度访问频率)auto            - [限制商品维度访问频率](#限制商品维度访问频率)auto        - [SOA 服务层优化](#soa-服务层优化)auto        - [秒杀整体流程图](#秒杀整体流程图)auto        - [总结](#总结)auto    - [二、数据库架构发展历程](#二数据库架构发展历程)auto        - [单机MySQL的美好年代](#单机mysql的美好年代)auto        - [Memcached(缓存)+MySQL+垂直拆分](#memcached缓存mysql垂直拆分)auto        - [Mysql主从复制读写分离](#mysql主从复制读写分离)auto        - [分表分库+水平拆分+mysql集群](#分表分库水平拆分mysql集群)auto    - [三、MySQL的扩展性瓶颈](#三mysql的扩展性瓶颈)auto    - [四、为什么要使用NOSQL   NOT ONLY SQL](#四为什么要使用nosql---not-only-sql)auto    - [五、传统RDBMS VS NOSQL](#五传统rdbms-vs-nosql)auto    - [六、NOSQL数据库的类型](#六nosql数据库的类型)auto    - [七、阿里巴巴中文站商品信息如何存放](#七阿里巴巴中文站商品信息如何存放)auto        - [商品基本信息](#商品基本信息)auto        - [商品描述、详情、评价信息(多文字类)](#商品描述详情评价信息多文字类)auto        - [商品的图片](#商品的图片)auto        - [商品的关键字](#商品的关键字)auto        - [商品的波段性的热点高频信息](#商品的波段性的热点高频信息)auto        - [商品的交易、价格计算、积分累计](#商品的交易价格计算积分累计)auto        - [大型互联网应用(大数据、高并发、多样数据类型)的难点和解决方案](#大型互联网应用大数据高并发多样数据类型的难点和解决方案)auto    - [八、数据的水平拆分和垂直拆分](#八数据的水平拆分和垂直拆分)auto        - [垂直拆分](#垂直拆分)auto        - [水平拆分](#水平拆分)auto        - [拆分原则](#拆分原则)auto        - [案例分析](#案例分析)auto    - [九、分布式事务](#九分布式事务)auto        - [假如没有分布式事务](#假如没有分布式事务)auto        - [什么是分布式事务？](#什么是分布式事务)auto        - [XA两阶段提交（2PC）](#xa两阶段提交2pc)auto        - [XA三阶段提交（3PC）](#xa三阶段提交3pc)auto        - [MQ事务](#mq事务)auto        - [TCC事务](#tcc事务)auto    - [十、BitMap](#十bitmap)auto        - [Bit-map的基本思想](#bit-map的基本思想)auto        - [Bit-map应用之快速排序](#bit-map应用之快速排序)auto        - [Bit-map应用之快速去重](#bit-map应用之快速去重)auto        - [Bit-map应用之快速查询](#bit-map应用之快速查询)auto        - [Bit-map扩展——Bloom Filter(布隆过滤器)](#bit-map扩展bloom-filter布隆过滤器)auto        - [总结](#总结-1)auto        - [应用](#应用)auto    - [十一、Bloom Filter](#十一bloom-filter)auto    - [十二、常见的限流算法](#十二常见的限流算法)auto        - [计数器法](#计数器法)auto        - [滑动窗口](#滑动窗口)auto        - [漏桶算法](#漏桶算法)auto        - [令牌桶算法](#令牌桶算法)auto        - [计数器 VS 滑动窗口](#计数器-vs-滑动窗口)auto        - [漏桶算法 VS 令牌桶算法](#漏桶算法-vs-令牌桶算法)auto    - [十三、负载均衡](#十三负载均衡)auto        - [dns域名解析负载均衡](#dns域名解析负载均衡)auto        - [反向代理负载均衡](#反向代理负载均衡)auto        - [http重定向协议实现负载均衡](#http重定向协议实现负载均衡)auto        - [分层的负载均衡算法](#分层的负载均衡算法)auto    - [十四、一致性Hash算法](#十四一致性hash算法)auto- [数据库](#数据库)auto    - [一、数据库范式](#一数据库范式)auto        - [1NF(第一范式)](#1nf第一范式)auto        - [2NF(第二范式)](#2nf第二范式)auto        - [3NF(第三范式)](#3nf第三范式)auto    - [二、数据库开发规范](#二数据库开发规范)auto        - [基础规范](#基础规范)auto        - [命名规范](#命名规范)auto        - [字段设计规范](#字段设计规范)auto        - [总结](#总结-2)auto    - [三、数据库索引](#三数据库索引)auto        - [唯一索引](#唯一索引)auto        - [非唯一索引](#非唯一索引)auto        - [主键索引](#主键索引)auto        - [聚集索引（聚簇索引）](#聚集索引聚簇索引)auto        - [扩展：聚集索引和非聚集索引的区别？分别在什么情况下使用？](#扩展聚集索引和非聚集索引的区别分别在什么情况下使用)auto        - [索引实现机制](#索引实现机制)auto        - [索引建立原则](#索引建立原则)auto    - [四、MyISAM vs InnoDB](#四myisam-vs-innodb)auto    - [五、并发事务带来的问题](#五并发事务带来的问题)auto        - [丢失更新](#丢失更新)auto        - [脏读（未提交读）](#脏读未提交读)auto        - [不可重复读](#不可重复读)auto        - [幻读（Phantom Read）](#幻读phantom-read)auto    - [六、事务隔离级别及锁的实现机制](#六事务隔离级别及锁的实现机制)auto        - [一级封锁协议 （对应 read uncommited）](#一级封锁协议-对应-read-uncommited)auto        - [二级封锁协议 （对应read commited)](#二级封锁协议-对应read-commited)auto        - [三级封锁协议 （对应reapetable read ）](#三级封锁协议-对应reapetable-read-)auto        - [最强封锁协议（对应Serialization）](#最强封锁协议对应serialization)auto    - [七、MVCC（多版本并发控制）](#七mvcc多版本并发控制)auto    - [八、间隙锁与幻读](#八间隙锁与幻读)auto        - [间隙锁（Next-Key锁）](#间隙锁next-key锁)auto        - [RR级别下防止幻读](#rr级别下防止幻读)auto- [设计模式与实践](#设计模式与实践)auto    - [一、OOP五大原则SOLID](#一oop五大原则solid)auto        - [单一责任原则](#单一责任原则)auto        - [开放封闭原则](#开放封闭原则)auto        - [里氏替换原则](#里氏替换原则)auto        - [依赖倒置原则](#依赖倒置原则)auto        - [接口分离原则](#接口分离原则)auto    - [二、设计模式](#二设计模式)auto    - [三、代理模式](#三代理模式)auto        - [定义与举例](#定义与举例)auto        - [静态代理](#静态代理)auto        - [动态代理](#动态代理)auto            - [JDK动态代理](#jdk动态代理)auto            - [CGLIB动态代理](#cglib动态代理)auto    - [四、面向切面编程（AOP）](#四面向切面编程aop)auto        - [基本思想](#基本思想)auto        - [登录验证](#登录验证)auto        - [基于RBAC的权限管理](#基于rbac的权限管理)auto            - [角色访问控制（RBAC）](#角色访问控制rbac)auto            - [执行流程分析](#执行流程分析)auto        - [日志记录](#日志记录)auto            - [日志记录最佳实践](#日志记录最佳实践)auto        - [事务处理](#事务处理)auto        - [统一异常处理](#统一异常处理)auto    - [五、工厂模式](#五工厂模式)auto        - [简单工厂](#简单工厂)auto        - [工厂方法](#工厂方法)auto        - [抽象工厂](#抽象工厂)auto    - [六、控制反转IOC](#六控制反转ioc)auto    - [七、观察者模式](#七观察者模式)auto    - [八、Zookeeper](#八zookeeper)auto        - [ZK简述](#zk简述)auto        - [存储结构](#存储结构)auto            - [znode](#znode)auto            - [znode中的存在类型](#znode中的存在类型)auto        - [应用场景](#应用场景)auto            - [统一命名服务](#统一命名服务)auto            - [负载均衡](#负载均衡)auto            - [统一配置管理](#统一配置管理)auto            - [集群管理](#集群管理)auto            - [服务器动态上下线](#服务器动态上下线)auto        - [写数据流程](#写数据流程)auto        - [Leader选举](#leader选举)auto- [数据结构与算法](#数据结构与算法)auto    - [一、 树](#一-树)auto    - [二、BST树](#二bst树)auto    - [**三、BST树**](#三bst树)auto    - [四、AVL树](#四avl树)auto    - [五、红黑树](#五红黑树)auto    - [六、B-树](#六b-树)auto    - [七、B+树](#七b树)auto    - [八、字典树](#八字典树)auto    - [九、跳表](#九跳表)auto    - [十、HashMap](#十hashmap)auto        - [简介](#简介)auto        - [内部实现](#内部实现)auto        - [存储结构-字段](#存储结构-字段)auto        - [功能实现-方法](#功能实现-方法)auto            - [1. 确定哈希桶数组索引位置](#1-确定哈希桶数组索引位置)auto            - [2. 分析HashMap的put方法](#2-分析hashmap的put方法)auto            - [3. 扩容机制](#3-扩容机制)auto        - [线程安全性](#线程安全性)auto    - [十一、ConcurrentHashMap](#十一concurrenthashmap)auto        - [锁分段技术](#锁分段技术)auto        - [CAS无锁算法](#cas无锁算法)auto            - [实现方式](#实现方式)auto            - [存在的缺点](#存在的缺点)auto    - [十二、ConcurrentLinkedQueue](#十二concurrentlinkedqueue)auto        - [延迟更新tail节点](#延迟更新tail节点)auto        - [延迟删除head节点](#延迟删除head节点)auto    - [十三、Topk问题](#十三topk问题)auto        - [简述](#简述)auto        - [解决方案](#解决方案)auto        - [实际运行](#实际运行)auto            - [（1）单机+单核+足够大内存](#1单机单核足够大内存)auto            - [（2）单机+多核+足够大内存](#2单机多核足够大内存)auto            - [（3）单机+单核+受限内存](#3单机单核受限内存)auto            - [（4）多机+受限内存](#4多机受限内存)auto        - [经常被提及的该类问题](#经常被提及的该类问题)auto        - [重复问题](#重复问题)auto    - [十四、资源池思想](#十四资源池思想)auto        - [作用](#作用)auto        - [线程池](#线程池)auto        - [连接池](#连接池)auto    - [十五、JVM内存管理算法](#十五jvm内存管理算法)auto        - [判断对象是否存活](#判断对象是否存活)auto            - [引用计数法](#引用计数法)auto            - [可达性分析算法](#可达性分析算法)auto        - [垃圾回收算法](#垃圾回收算法)auto            - [标记-清除算法(Mark-Sweep)](#标记-清除算法mark-sweep)auto            - [复制算法(Copying)](#复制算法copying)auto            - [标记-整理算法(Mark-Compact)](#标记-整理算法mark-compact)auto            - [分代收集算法(Generational Collection)](#分代收集算法generational-collection)auto    - [十六、容器虚拟化技术，Doocker思想](#十六容器虚拟化技术doocker思想)auto        - [为什么会有docker](#为什么会有docker)auto        - [docker理念](#docker理念)auto        - [实现方式](#实现方式-1)auto        - [docker的组成](#docker的组成)auto            - [镜像](#镜像)auto            - [容器](#容器)auto            - [仓库](#仓库)auto            - [总结](#总结-3)auto    - [十七、持续集成、持续发布，jenkins](#十七持续集成持续发布jenkins)auto        - [持续集成](#持续集成)auto        - [手动部署](#手动部署)auto        - [自动部署](#自动部署)auto- [面试题举例](#面试题举例)auto    - [一、设计一个分布式环境下全局唯一的发号器](#一设计一个分布式环境下全局唯一的发号器)auto            - [1、UUID](#1uuid)auto            - [2、数据库自增长序列或字段](#2数据库自增长序列或字段)auto            - [3、数据库sequence表以及乐观锁](#3数据库sequence表以及乐观锁)auto            - [4、Redis生成ID](#4redis生成id)auto            - [5、Twitter的snowflake算法](#5twitter的snowflake算法)auto    - [二、设计一个带有过期时间的LRU缓存](#二设计一个带有过期时间的lru缓存)auto        - [问题描述](#问题描述)auto        - [问题分析](#问题分析)auto        - [过期时间实现](#过期时间实现)auto            - [维护一个线程](#维护一个线程)auto            - [惰性删除](#惰性删除)auto    - [三、设计一个分布式锁](#三设计一个分布式锁)auto        - [什么是分布式锁？](#什么是分布式锁)auto        - [我们需要怎样的分布式锁？](#我们需要怎样的分布式锁)auto        - [基于数据库做分布式锁](#基于数据库做分布式锁)auto            - [1、基于乐观锁](#1基于乐观锁)auto            - [2、基于悲观锁](#2基于悲观锁)auto        - [基于 Redis 做分布式锁](#基于-redis-做分布式锁)auto            - [1、基于 redis 的 setnx()、expire() 方法做分布式锁](#1基于-redis-的-setnxexpire-方法做分布式锁)auto            - [2、基于 redis 的 setnx()、get()、getset()方法做分布式锁](#2基于-redis-的-setnxgetgetset方法做分布式锁)auto        - [基于 ZooKeeper 做分布式锁](#基于-zookeeper-做分布式锁)auto        - [使用分布式锁的注意事项](#使用分布式锁的注意事项)auto        - [分布式可重入锁的设计](#分布式可重入锁的设计)auto    - [四、设计一个分布式环境下的统一配置中心](#四设计一个分布式环境下的统一配置中心)auto        - [配置中心概述](#配置中心概述)auto        - [演进中的配置](#演进中的配置)auto        - [配置中心之简版](#配置中心之简版)auto        - [配置中心之性能改进](#配置中心之性能改进)auto        - [配置中心之可用性改进](#配置中心之可用性改进)auto    - [五、如何准备HR面试](#五如何准备hr面试)autoauto<!-- /TOC -->
